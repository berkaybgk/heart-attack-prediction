...
def train_xgboost(X_train, y_train, scale_pos_weight=None, **kwargs):
    """
    ...
    scale_pos_weight : float, optional
        Weight for positive class to handle imbalance
    **kwargs : dict
...
def plot_feature_importance(model, feature_names, top_n=20, save_path=None):
    ...
    # Get feature importance (using 'weight' - number of times feature appears in trees)
    ...
    # Normalize importances to sum to 1 for better interpretation
    if top_importances.sum() > 0:
        top_importances_norm = top_importances / top_importances.sum()
    else:
        top_importances_norm = top_importances
    
    plt.figure(figsize=(10, 8))
    plt.barh(range(top_n), top_importances_norm[::-1], align='center', color='steelblue')
    ...
    print("-"*60)
    for i, (feature, importance) in enumerate(zip(top_features, top_importances_norm), 1):
...
def save_results_summary(model, evaluation_results, save_path):
    ...
    with open(save_path, 'w') as f:
        f.write("="*60 + "\n")
        f.write("XGBOOST MODEL - HEART ATTACK PREDICTION\n")
        f.write("="*60 + "\n")
...
def main():
    ...
    xgb_model = train_xgboost(
        X_train, 
        y_train,
        n_estimators=1100,                   # Number of boosting rounds
        max_depth=5,                         # Maximum depth of trees
        learning_rate=0.008,                 # Step size shrinkage
        random_state=42,
    )
    
    # Evaluate model
    evaluation_results = evaluate_model(xgb_model, X_train, X_test, y_train, y_test)
    
    # Plot confusion matrix for test set
    plot_confusion_matrix(
        evaluation_results['test_cm'],
        'Confusion Matrix - Test Set (XGBoost)',
        save_path=os.path.join(RESULTS_DIR, 'confusion_matrix_test_xgboost.png')
    )
    ...
    plot_roc_curve(
        ...
        'ROC Curve - Test Set (XGBoost)',
        save_path=os.path.join(RESULTS_DIR, 'roc_curve_test_xgboost.png')
    )
    ...
    plot_feature_importance(
        ...
        top_n=20,
        save_path=os.path.join(RESULTS_DIR, 'feature_importance_xgboost.png')
    )
    ...
    save_results_summary(
        xgb_model,
        evaluation_results,
        os.path.join(RESULTS_DIR, 'results_summary_xgboost.txt')
    )
    ...
    return xgb_model, evaluation_results
