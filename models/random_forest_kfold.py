...
def train_and_evaluate_kfold(X_train, y_train, n_splits=5, random_state=42, **model_params):
    """
    Train Random Forest using k-fold cross validation.
    ...
    **model_params : dict
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(X_train, y_train), 1):
        ...
        # Make predictions
        y_fold_train_pred = rf_model.predict(X_fold_train)
        y_fold_val_pred = rf_model.predict(X_fold_val)
        
        y_fold_train_proba = rf_model.predict_proba(X_fold_train)[:, 1]
        y_fold_val_proba = rf_model.predict_proba(X_fold_val)[:, 1]
        ...
        # Store results
        fold_results['models'].append(rf_model)
        ...
        fold_results['feature_importances'].append(rf_model.feature_importances_)
...
def plot_feature_importance(fold_results, feature_names, top_n=20, save_path=None):
    ...
    plt.xlabel('Average Feature Importance (across folds)', fontsize=12)
...
def save_results_summary(fold_results, train_summary, val_summary, 
    ...
    with open(save_path, 'w') as f:
        f.write("="*60 + "\n")
        f.write("RANDOM FOREST WITH K-FOLD CROSS VALIDATION\n")
...
def main():
    ...
    RESULTS_DIR = '../results/random_forest_kfold'
    ...
    ensemble_save_path = os.path.join(RESULTS_DIR, 'rf_ensemble_models.pkl')
